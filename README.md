...
- **Privacy:** Local CosyVoice keeps readable text on the device. Every online mode sends text to its service provider. The `edge-tts` interface used here does not provide an explicit ZDR guarantee for plugin requests. OpenRouter requests force `provider.zdr = true`, deny provider data collection, and fail instead of relaxing those restrictions when no eligible endpoint exists. This supports its four-star relative rating among the available modes, but it is not equivalent to local processing: text still passes through OpenRouter and an upstream provider, and account-level logging and data sharing should remain disabled for private content.

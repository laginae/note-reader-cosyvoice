...
完整设置页面，由同一页面的连续视图片段拼接而成。截图不包含密钥值、Obsidian 库名、笔记内容、本地路径或周围的 Obsidian 界面：

![Note and PDF Voice Reader 设置页](docs/images/plugin-settings.zh-CN.png)

## 语音引擎对比

下表是针对本插件实际使用体验的相对评分。实心星越多始终代表越有利：隐私保护更强、配置更方便、首段启动更快、音色选择更多、对用户付费 API 额度依赖更小或离线能力更强。`★★★★★` 为最高评分，`★☆☆☆☆` 为最低评分。评分仅用于选型参考，不构成安全性、服务商政策、价格或延迟保证。

| 模式 | 隐私保护 | 设置便利性 | 启动速度 | 音色选择 | 额度友好度 | 离线能力 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: |
| 本地 CosyVoice | ★★★★★ | ★★☆☆☆ | ★★★★☆ | ★★★☆☆ | ★★★★★ | ★★★★★ |
| Microsoft Edge 在线语音 | ★★★☆☆ | ★★★★☆ | ★★★★★ | ★★★★★ | ★★★★★ | ★☆☆☆☆ |
...
- **额度友好度：** 本地模式使用本机算力而不依赖远程 API 额度，因此获得五星。Edge 在本插件中不需要用户 API 密钥或自行管理付费额度，也获得五星，但这不表示服务无限量或具有可用性保证。Azure 获得四星，因为其当前的[免费 F0 层级](https://azure.microsoft.com/en-us/pricing/details/speech/)列出每月 50 万个神经文本转语音字符；实时不保留是独立的隐私属性，并不是名为“免费 ZDR 额度”的产品，而且价格和资格以后可能变化。OpenRouter 获得三星，因为 TTS [按输入字符计量](https://openrouter.ai/docs/guides/overview/multimodal/tts)，价格取决于模型和供应商；插件会强制 ZDR，但不会假定长期存在免费 ZDR TTS 额度。

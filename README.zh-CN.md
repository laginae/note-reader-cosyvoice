...
- 支持暂停、继续、停止；控制面板获得焦点时可用空格暂停/继续、左右方向键前后跳转 5 秒；也支持在当前已加载音频块内点击或拖动进度条。
...
右侧边栏 `CosyVoice Reader` 控制面板获得焦点时，空格可以暂停或继续朗读，左方向键后退 5 秒，右方向键前进 5 秒。方向键跳转范围限制在当前已加载音频块内。
